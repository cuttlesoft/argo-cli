...
CLI.projectPath = function projectPath(marker) {
  // marker or defaults to 'cuttle.project', but may be a git/hg repo file/dir instead
  const projectFile = marker || 'cuttle.project';
...
CLI.readProjectFile = function readProjectFile(projectPath, callback) {
  const projectFile = `${projectPath}/cuttle.project`;
...
function dotDiff(leftString, dotsLength) {
...
function printTaskSummary(task, indent, offset) {
  ...
  // const separatorSize = name.length <= offset ? offset - name.length : 0;
  const separator = dotDiff(name, offset); // '.' * separatorSize;

  w(`${' ' * indent}${name.green.bold}  ${separator.grey}  ${summary.bold}\n`);
}
...
CLI.printAvailableTasks = function printAvailableTasks() {
  ...
  // function writeSummary(task) {
  //   if (task.summary) {
  //     const dotsCount = task.name.length <= 15 ? 15 - task.name.length : 0;
  //     const dots = `${'.' * dotsCount}`;
  //
  //     wErr(`   ${task.name.green.bold}  ${dots.grey}  ${task.summary.bold}\n`);
  //   }
  // }

  TASKS.forEach(task => printTaskSummary(task));
  wErr('\n');
};
...
CLI.printUsage = function printUsage(task) {
  ...
  function printArg(arg) {
    ...
    const argIndent = `${indent}${' ' * arg.length}`;
    const argSeparator = `\n${argIndent}`;
  ...
  function printOpt(opt) {
    // ??? : 242 - 250
    const nameIndent = ' ' * task.name.length;
    // const optIndent = `${nameIndent}${' ' * subIndent}`;

    // ??? : 252 - 264
    const optLine = `${nameIndent}[${opt}]`;
    const optSeparator = dotDiff(optLine, rightColumn);
