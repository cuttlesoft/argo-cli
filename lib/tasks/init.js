...
function projectExists(location) {
  const existence = fs.existsSync(`${location}/argo.project`);
...
ArgoTask.prototype.run = function run(argo) {
  ...
  function answer(answers) {
    ...
    if (!repoExists) {
      ...
      // check for existing argo.project with slug now available; quit if found
    ...
    function createProjectFile(err, res) {
      ...
      function startCommit() {
        handleError(err);
        // initial commit with argo.project file
  ...
  if (repoExists) {
    // check for existing argo.project; quit if found
    if (projectExists(projectPath)) return;
